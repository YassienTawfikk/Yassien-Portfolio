document.addEventListener("DOMContentLoaded", function () {
                ...
                page.getAnnotations().then(annotations => {
                    annotations.forEach(annotation => {
                        if (annotation.subtype === 'Link' && annotation.rect) {
                            // Annotation rect is [x, y, x2, y2] in PDF coords (bottom-left origin)
                            // Viewport.convertToViewportRectangle handles logic
                            const rect = layerViewport.convertToViewportRectangle(annotation.rect);
                            // rect is [x, y, x2, y2] (top-left origin after conversion usually)
                            // Actually it returns [minX, minY, maxX, maxY]
                            ...
                            // Debug border? No, keep invisible
    ...
    // Updates layout using cursor position to compensate scroll
    function updateLayout(newZoom, clientX, clientY) {
        ...
        // 1. Capture State relative to Cursor BEFORE resize
        const wrapperRect = zoomWrapper.getBoundingClientRect();

        // Offset of cursor relative to the wrapper's top-left
        let offsetX = 0;
        let offsetY = 0;
        ...
        if (clientX !== undefined && clientY !== undefined) {
            ...
            // Default center of content? 
            // Or center of viewport?
            // Use center of viewport as default anchor
        ...
        // 4. Compensate Scroll
        // The point at `offsetX` moved to `offsetX * ratio`.
        // The shift is `offsetX * ratio - offsetX` => `offsetX * (ratio - 1)`.
